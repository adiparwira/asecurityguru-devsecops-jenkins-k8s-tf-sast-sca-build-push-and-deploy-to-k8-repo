pipeline {
  ...
  tools { 
   ...
   stages{
	...
	stage('Build') { 
            steps { 
               withDockerRegistry([credentialsId: "MY_DOCKER_HUB", url: ""]) {
                 script{
	...
	stage('Push') {
            steps {
                script{
                    docker.withRegistry('https://118160814754.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:aws-credentials') {
                    app.push("latest")
                    }
                }
            }
